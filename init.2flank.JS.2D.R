...
init.2flank.JS.2D <- function(data=data,M=M,n.fixed=NA,initTrue=FALSE){
  X <- data$X
  ...
  if(initTrue){#initialize flanks to true matches?
    ...
    # #used to initialize
    # n.cap <- n.fixed + n.L-n.fixed + n.R-n.fixed #number unique inds captured given IDs
    # N.super.init <- n.cap
    # if(N.super.init >= M){
    #   M <- n.cap + 50
    #   warning("Must augment more than number of individuals captured implied by initialized IDs. Arbitrarily setting M to 50 more than that")
    # } 
